...
# Enable mixed precision (add at the start of the file)
...
def weighted_focal_loss(y_true, y_pred, alpha=0.25, gamma=2.0, class_weights=None):
    ...
    # Apply class weights - reshape weights to match broadcasting
    if class_weights is not None:
        class_weights = tf.convert_to_tensor(class_weights)
        class_weights = tf.cast(class_weights, tf.float32)
        # Reshape to [1, num_classes] for proper broadcasting
        class_weights = tf.reshape(class_weights, [1, -1])
        weighted_ce = tf.reduce_sum(y_true * class_weights * ce[..., tf.newaxis], axis=-1)
    else:
        weighted_ce = ce
